...
const FileParser = () => {
  ...
  if (data1.length > 0) {
    ...
    drops = (
      <div>
        <DropList name='X-Axis type' data={data1} />
        <DropList name='Y-Axis type' data={data1} />
        <DropList name='Chart type' data={['bar', 'lines', 'scatter', 'pie']} />
      </div>
    );
  }
      ...
      chart = (
        <Plot
          ...
          layout={{
            width: 1000,
            height: 500,
            title: data.plotType + ' ' + data.xName + ' vs. ' + data.yName,
            showlegend: true,
            xaxis: { title: data.xName },
            yaxis: { title: data.yName },
          }}
        />
      );
